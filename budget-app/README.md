...
기존에 Supabase 프로젝트를 만들어 쓰고 있었다면, **가구 공유** 기능을 쓰기 위해 [`supabase/schema.sql`](./supabase/schema.sql)을 SQL Editor에서 한 번 더 실행해야 합니다 (households/household_members/household_invites 테이블과 RLS 정책이 추가되며, 기존 사용자에게는 자동으로 개인 가구가 배정됩니다).
